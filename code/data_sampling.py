...
def dataNormalize(sampling_frame_num=10, sampling_frame_iter=30):

    # 파일 경로를 현재 위치의 input pose에서 영상 번호별로 가져온다.
    # 그리고 그걸 리스트에 담는다.
    inputData_dir = "../data/3_dataFrame/"
    ...
    for k in range(len(videoData_dirs)):
        ...
        if os.path.exists(
            "../data/4_data_sampling/" + os.path.basename(videoData_dirs[k])
        ):
        ...
        videoOutput_dir = "../data/4_data_sampling/" + os.path.basename(
            videoData_dirs[k]
        )
        ...
        print("{} 파일을 읽었습니다.".format(filelist[0]))
        ...
        framePer1Unit = int(secondPer1Unit * fps)
        ...
        print("샘플링으로 뽑을 갯수 : {}".format(sampling_frame_num))
        print("한 마디 내에서 몇번을 샘플링 할 것인지 : {}".format(sampling_frame_iter))
        time.sleep(3)
...
if __name__ == "__main__":
    ...
    #             ) as make_file:
    #                 json.dump(json_data, make_file, indent="\t")
    #                 print(json.dump(json_data, make_file, indent="\t"))
    #                 time.sleep(10)
    #             continue
    ...
    #         poseData_norm[:, 1] = (poseData_norm[:, 1] - mean_y) / std_y
    ...
    #             os.path.basename(videoData_dirs[k]), len(filelist), numbering
    #         ),
    #         end="\n\n",
    #     )
    #     time.sleep(10)
